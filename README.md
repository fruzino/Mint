...
## 8. Developer API 
    ...
    Since some of you may want to create a plugin for Mint, here is the simple steps to do it.

    Create a new folder inside "plugins" folder
    Create a 2 files inside that new folder

    yourpluginname.py
    yourpluginname.plug

    In the .py enter your command, one example is this >>> 

        def cmd_mycommand(args):
            print(f"You said: {args[0]}")

        DISPATCH = { "mycommand": cmd_mycommand }
...
## 9. Updates(Contributers Only)
    ...
    For the website... do git pushing to origin...
